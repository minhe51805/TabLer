use super::export::{
    ...
    DatabaseExportSnapshotTable, ExportTableBundle, SqlExportPayload, DATABASE_EXPORT_BATCH_SIZE,
...
pub(super) async fn build_sql_export(
    ...
    for bundle in &ordered_tables {
        ...
        loop {
            ...
            if (batch.rows.len() as u64) < DATABASE_EXPORT_BATCH_SIZE {
                break;
            }
        }
        if db_type == DatabaseType::MSSQL {
...
pub(super) fn ensure_trailing_semicolon(statement: &str) -> String {
...
pub(super) fn build_insert_statement_batch(
    db_type: DatabaseType,
    table_ref: &str,
    columns: &[ColumnInfo],
    rows: &[Vec<JsonValue>],
