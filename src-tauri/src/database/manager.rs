...
impl DatabaseManager {
    pub fn new() -> Self {
        // A broken plugin-storage location must not take the whole app down at
        // boot: degrade to a temporary directory and let plugin commands surface
        // the degraded state when actually used.
        let plugin_storage = match PluginStorage::new() {
            Ok(storage) => storage,
            Err(error) => {
                log::error!(
                    "TableR plugin storage could not be initialized: {error}; falling back to a temporary directory"
                );
                match PluginStorage::from_data_dir(
                    std::env::temp_dir().join("tabler-plugin-storage-fallback"),
                ) {
                    Ok(storage) => storage,
                    Err(fallback_error) => panic!(
                        "TableR plugin storage is unusable: {error}; temporary fallback failed too: {fallback_error}"
                    ),
                }
            }
        };
        Self::with_plugin_storage(plugin_storage)
    }
