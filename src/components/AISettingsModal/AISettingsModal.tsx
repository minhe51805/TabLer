...
export function AISettingsModal({ onClose }: Props) {
  ...
  const [localOllamaConsentTone, setLocalOllamaConsentTone] = useState<
    "info" | "success" | "error"
  >("info");
  ...
  const handleToggleModelDisabled = (index: number) => {
    ...
    if (isDisabling && activeConfig.model === modelName) {
      nextModel =
        activeConfigModels.find((entry) => entry !== modelName && !disabled.has(entry)) ?? "";
    }
  ...
  const handleSaveModelDialog = () => {
    ...
    const duplicateIndex = activeConfigModels.findIndex(
      (entry, entryIndex) =>
        entryIndex !== modelDialog.index && entry.toLowerCase() === value.toLowerCase(),
    );
    if (duplicateIndex !== -1) {
    ...
    const models = [...activeConfigModels];
    const wasActiveModel =
      modelDialog.index >= 0 && models[modelDialog.index] === activeConfig.model;
    ...
    const disabledModels = (activeConfig.disabled_models ?? []).map((entry) =>
      previousName && entry === previousName ? value : entry,
    );
  ...
  const handleConfirmModelPicker = () => {
    ...
    const toAdd = modelPicker.models
      .filter(
        (entry) =>
          modelPicker.selected.includes(entry.id) && !existingLower.has(entry.id.toLowerCase()),
      )
      .map((entry) => entry.id);
  ...
  const hasStoredKey = activeConfig
    ? storedKeyStatus[activeConfig.id] && !clearedKeyIds.includes(activeConfig.id)
    : false;
  ...
  return (
    <div className="ai-settings-overlay">
      <div className="ai-settings-modal">
        ...
        <header className="ai-settings-header">
          ...
          <div className="ai-settings-header-actions">
            ...
            <button
              type="button"
              onClick={handleSave}
              className="ai-settings-btn-save"
              disabled={disableModalActions}
            >
              {isSaving ? (
        ...
        {saveError && <div className="ai-settings-error">{saveError}</div>}
              ...
              <div className="ai-settings-local-card">
                <div className="ai-settings-local-copy">
                  ...
                  <p className="ai-settings-local-description">
                    One click will install Ollama if needed, download <code>gemma4:e2b</code>{" "}
                    locally, and switch TableR to use that model first for workspace AI.
                  </p>
                  <div className="ai-settings-local-badges">
                    ...
                    <span className="ai-settings-chip">
                      {localOllamaStatus?.hasRecommendedModel
                        ? "Model ready"
                  ...
                  <div className="ai-settings-local-meta">
                    ...
                    <span>
                      Endpoint:{" "}
                      {localOllamaStatus?.endpoint || "http://localhost:11434/v1/chat/completions"}
                    </span>
                  </div>
                    ...
                    <div className="ai-settings-progress-meta">
                      <span>
                        {isSettingUpLocalOllama
                          ...
                          : "Windows setup uses the official Ollama installer."}
                      </span>
                ...
                <header className="ai-settings-detail-header">
                  {isEditingName ? (
                    ...
                    <h3 className="ai-settings-detail-title" title={activeConfig.name}>
                      {activeConfig.name ||
                        formatAIProviderTypeLabel(activeConfig.provider_type) ||
                        "Unnamed"}
                    </h3>
                  )}
                  <button
                  ...
                  <span
                    className={`ai-settings-status-pill ${activeConfig.is_enabled ? "is-on" : "is-off"}`}
                  >
                  ...
                  <div className="ai-settings-fields">
                          ...
                          <div className="ai-settings-select-menu">
                            {AI_PROVIDER_TYPES.map((value) => (
                              <button
                                ...
                                onClick={() => {
                                  updateConfig(activeConfig.id, {
                                    provider_type: value as AIProviderConfig["provider_type"],
                                  });
                                  setIsProviderMenuOpen(false);
                                }}
                              >
                    ...
                    <div className="ai-settings-field">
                      ...
                      <div className="ai-settings-model-list">
                        {activeConfigModels.length > 0 ? (
                          activeConfigModels.map((modelName, index) => {
                            const isModelDisabled = (activeConfig.disabled_models ?? []).includes(
                              modelName,
                            );
                            return (
                              <div
                                key={`${modelName}-${index}`}
                                className={`ai-settings-model-row ${isModelDisabled ? "is-disabled" : ""}`}
                              >
                                <span
                                  className={`ai-settings-model-name ${modelName === activeConfig.model ? "is-active" : ""}`}
                                >
                                  {modelName}
                                </span>
                                {isModelDisabled ? (
                                  <span className="ai-settings-model-inactive">Disabled</span>
                                ) : null}
                                {modelName === activeConfig.model && !isModelDisabled ? (
                                    ...
                                    setModelDialog({
                                      ...
                                      contextWindow:
                                        existing?.context_window != null
                                          ? String(existing.context_window)
                                          : "",
                                      maxOutputTokens:
                                        existing?.max_output_tokens != null
                                          ? String(existing.max_output_tokens)
                                          : "",
                    ...
                    <div className="ai-settings-field">
                      <label className="ai-settings-label">Fast model</label>
                      <input
                        className="ai-settings-input"
                        type="text"
                        list={`ai-fast-models-${activeConfig.id}`}
                        value={activeConfig.fast_model ?? ""}
                        placeholder="Optional — e.g. a cheaper model for simple asks"
                        onChange={(e) =>
                          updateConfig(activeConfig.id, {
                            fast_model: e.target.value.trim() || null,
                          })
                        }
                        disabled={isSettingUpLocalOllama}
                      />
                      <datalist id={`ai-fast-models-${activeConfig.id}`}>
                        {activeConfigModels.map((modelName) => (
                          <option key={modelName} value={modelName} />
                        ))}
                      </datalist>
                      <p className="ai-settings-model-empty">
                        Trivial asks (short explains, formatting, general chat) use this model when
                        set; everything else uses the active model.
                      </p>
                    </div>
                    {activeConfig.provider_type === "custom" ? (
                      <div className="ai-settings-field">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            updateConfig(activeConfig.id, {
                              api_format: e.target.value === "auto" ? null : e.target.value,
                            })
                          }
                          disabled={isSettingUpLocalOllama}
                        >
                    ...
                    {modelDialog ? (
                      <div
                        className="ai-settings-model-dialog-backdrop"
                        role="dialog"
                      ...
                      >
                        <div className="ai-settings-model-dialog">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setModelDialog({ ...modelDialog, value: e.target.value })
                            }
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setModelDialog({ ...modelDialog, maxOutputTokens: e.target.value })
                            }
                          ...
                          {modelDialogError ? (
                            <div className="ai-settings-error">{modelDialogError}</div>
                          ) : null}
                        ...
                        <div className="ai-settings-model-dialog is-picker">
                          ...
                          <div className="ai-settings-model-dialog-actions">
                            ...
                            <button
                              type="button"
                              className="ai-settings-btn-save"
                              onClick={handleConfirmModelPicker}
                            >
                              Add selected
                            </button>
                          </div>
                    ...
                    <div className="ai-settings-field">
                      <label className="ai-settings-label">
                        {endpointFieldCopy?.label || "Custom URL"}
                      </label>
                      <input
                        type="text"
                        value={activeConfig.endpoint}
                        onChange={(e) =>
                          updateConfig(activeConfig.id, { endpoint: e.target.value })
                        }
                      ...
                      {activeConfig.provider_type !== "custom" &&
                        activeConfig.endpoint.trim().length > 0 && (
                          <button
                            type="button"
                ...
                <div className="ai-settings-section">
                  ...
                  <div className="ai-settings-primary-row">
                    <span
                      ...
                      title={connectionCheckMessage || undefined}
                    >
                  ...
                  <p className="ai-settings-field-hint">
                    "Use for AI" makes this provider the active assistant for chat and SQL
                    completion.
                  </p>
                </div>
                {/* Access */}
                    ...
                    <div className="ai-settings-toggle-row">
                      ...
                      <label className="ai-settings-toggle-switch">
                        <input
                          ...
                          onChange={(e) =>
                            updateConfig(activeConfig.id, {
                              allow_schema_context: e.target.checked,
                            })
                          }
                          disabled={isSettingUpLocalOllama}
                        />
                    ...
                    <div className="ai-settings-toggle-row">
                      ...
                      <label className="ai-settings-toggle-switch">
                        <input
                          ...
                          onChange={(e) =>
                            updateConfig(activeConfig.id, {
                              allow_inline_completion: e.target.checked,
                            })
                          }
                          disabled={isSettingUpLocalOllama}
                        />
            ...
            <div className="ai-settings-consent-dialog">
              <div className="ai-settings-consent-copy">
                ...
                <p className="ai-settings-consent-description">
                  TableR will use the official Ollama installer for Windows, start the local
                  service, download <code>gemma4:e2b</code> to this machine, and switch the app to
                  that local model.
                </p>
                <div className="ai-settings-consent-list">
                  <div className="ai-settings-consent-item">
                    Downloads roughly 7.2 GB for the model.
                  </div>
                  {localOllamaStatus?.version && (
                    <div className="ai-settings-consent-item">
                      Current Ollama version detected: <code>{localOllamaStatus.version}</code>.
                    </div>
                  )}
                  <div className="ai-settings-consent-item">
                    Keeps inference local at <code>localhost:11434</code>.
                  </div>
                  <div className="ai-settings-consent-item">
                    Makes the Ollama provider active in TableR after setup.
                  </div>
                </div>
                <div
                  className={`ai-settings-consent-progress ai-settings-consent-progress-${localOllamaConsentTone}`}
                >
                  <div className="ai-settings-progress-meta">
                    <span>
                      {isSettingUpLocalOllama
                        ? localOllamaProgress.message
                        : localOllamaConsentNotice ||
                          "Windows may ask for permission while Ollama installs."}
                    </span>
                    <strong>{localOllamaProgressLabel}</strong>
                  </div>
                  <div className="ai-settings-progress-track">
