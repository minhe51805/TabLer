...
export interface AIWorkspacePanelViewModel {
  ...
  attachedSelection: SelectionContextState | null;
  bubbleCountByThread: Map<string, number>;
  composerFooterNote: string;
  composerRef: RefObject<HTMLDivElement | null>;
  composerTextareaRef: RefObject<HTMLTextAreaElement | null>;
  connectionId: string | null;
  conversationBubbles: AIWorkspaceBubbleData[];
  currentDatabase: string | null;
  currentThread: AIChatThread | null;
  deleteThreadPending: string | null;
  ...
  language: string;
  promptDraft: string;
  recentWorkspaceThreads: AIChatThread[];
  renameThread: (threadId: string, label: string) => void;
  sessionDataReadButtonLabel: string;
  sessionDataReadButtonTitle: string;
  showThinking: boolean;
  switchableProviders: AIProviderConfig[];
  tableContextCount: number;
  visibleError: string | null;
  visualizationConsentPending: ConfirmState | null;
  destructiveConsentPending: ConfirmState | null;
  failoverConsentPending: ConfirmState | null;
  chatThreadRef: RefObject<HTMLDivElement | null>;
  contextUsage: { used: number; limit: number };
  activeChatWorkspaceId: string | null;
  activeChatWorkspaceName: string | null;
  activeChatWorkspaceContextUpdatedAt: number | null;
  chatWorkspaces: { id: string; name: string; contextUpdatedAt: number | null }[];
  importableChatThreads: AIChatThread[];
  ...
  compactContext: () => void;
  selectChatWorkspace: (id: string | null) => void;
  createChatWorkspace: () => void;
  renameChatWorkspace: (id: string, name: string) => void;
  deleteChatWorkspace: (id: string) => void;
  importChatThreads: (ids: string[]) => void;
  databases: string[];
  rebindChatWorkspace: (id: string, database: string) => void;
  generate: () => void;
  cancelGeneration: () => void;
  openSettings: () => void;
  requestDeleteThread: (id: string, event: React.MouseEvent) => void;
  /** One-click reply to an agent ask_user question. */
  sendAskUserReply: (option: string) => void;
  ...
  insertBubble: (bubble: AIWorkspaceBubbleData) => void;
  reset: () => void;
  selectThread: (id: string) => void;
  setHistoryOpen: (value: boolean | ((value: boolean) => boolean)) => void;
  setPromptDraft: (value: string) => void;
  setSessionDataReadEnabled: (value: boolean) => void;
  ...
  selectAgentAutonomy: (value: AIWorkspaceAgentAutonomy) => void;
  selectInteractionMode: (value: AIWorkspaceInteractionMode) => void;
  activateProvider: (id: string, model?: string) => void;
  toggleModelVisibility: (id: string, model: string) => void;
  confirmVisualizationConsent: (value: boolean) => void;
  confirmDestructiveConsent: (value: boolean) => void;
  resolveFailoverConsent: (approved: boolean) => void;
  cancelDeleteThread: () => void;
  composerKeyDown: KeyboardEventHandler<HTMLTextAreaElement>;
}
    ...
    <div className="ai-workspace-overlay">
      {m.visibleError && (
        <div className="ai-workspace-alert">
          <span>{m.visibleError}</span>
          <button type="button" className="ai-workspace-alert-dismiss" onClick={m.dismissError}>
            {m.aiCopy.composer.alertDismiss}
          </button>
        </div>
      )}
      <div className="ai-workspace-stage ai-workspace-stage--sidebar">
        <aside
          className={`ai-workspace-sidebar ${m.isLongformComposer ? "is-longform" : ""}`}
          style={{ width: panelWidth }}
        >
          <div
            className="ai-workspace-resize-handle"
            role="separator"
            aria-orientation="vertical"
            aria-valuenow={panelWidth}
            aria-valuemin={AI_PANEL_MIN_WIDTH}
            aria-valuemax={AI_PANEL_MAX_WIDTH}
            aria-label={m.aiCopy.composer.resizeHandleTitle}
            title={m.aiCopy.composer.resizeHandleTitle}
            onMouseDown={onResizeHandleMouseDown}
            onDoubleClick={() => setPanelWidth(AI_PANEL_DEFAULT_WIDTH)}
          >
            <div className="ai-workspace-resize-handle-line" />
          </div>
          <div
            ref={m.composerRef}
            className={`ai-workspace-composer is-docked ${m.isLongformComposer ? "is-longform" : ""} ${m.activeInteractionMode === "agent" ? "is-agent" : ""}`}
          >
            <div className="ai-workspace-composer-body">
              <header className="ai-workspace-panel-header workspace-toolbar">
                <div className="workspace-toolbar-main ai-workspace-panel-header-main">
                  <span className="workspace-toolbar-kicker">{m.aiCopy.composer.kicker}</span>
                  <div className="workspace-toolbar-title-row ai-workspace-panel-header-row">
                    <span className="workspace-toolbar-title">{m.aiCopy.composer.title}</span>
                  </div>
                </div>
                <div className="workspace-toolbar-actions">
                  ...
                  <button
                    ...
                    onClick={m.compactContext}
                    disabled={m.isCompacting}
                    title={
                      m.isCompacting
                        ? m.aiCopy.workspace.compactRunning
                        : m.aiCopy.workspace.compactAction
                    }
                  >
                  ...
                  <button
                    type="button"
                    className="toolbar-btn icon-only"
                    onClick={() => setChatActionModalOpen(true)}
                    title={m.aiCopy.composer.newChatTitle}
                  >
                    <MessageSquarePlus className="w-3.5 h-3.5" />
                  </button>
                  <button
                    type="button"
                    className="toolbar-btn icon-only is-close"
                    onClick={m.close}
                    title={m.aiCopy.composer.alertDismiss}
                  >
                    <X className="w-3.5 h-3.5" />
                  </button>
                </div>
              </header>
              <div className="ai-workspace-chat-tabs">
                <AIWorkspaceSwitcher
                  copy={m.aiCopy.workspace}
                  workspaces={m.chatWorkspaces}
                  activeWorkspaceId={m.activeChatWorkspaceId}
                  onSelectWorkspace={m.selectChatWorkspace}
                  onCreateWorkspace={m.createChatWorkspace}
                  onRenameWorkspace={m.renameChatWorkspace}
                  onDeleteWorkspace={m.deleteChatWorkspace}
                  databases={m.databases}
                  currentDatabase={m.currentDatabase}
                  onRebindWorkspace={m.rebindChatWorkspace}
                  rebindLocked={m.isGenerating || m.isRunning}
                  rebindLockedTitle={m.aiCopy.workspace.rebindLockedTitle}
                />
                <div className="ai-workspace-chat-toolbar-actions">
                  <div
                    ref={m.historyPanelRef}
                    className={`ai-workspace-history-dropdown ${m.isHistoryOpen ? "is-open" : ""}`}
                  >
                    <button
                      type="button"
                      className="ai-workspace-history-toggle"
                      onClick={() => m.setHistoryOpen((value) => !value)}
                      title={m.aiCopy.composer.historyTitle}
                      aria-label={m.aiCopy.composer.historyTitle}
                    >
                      <History className="w-3.5 h-3.5" />
                      <span className="ai-workspace-history-toggle-count">
                        {m.recentWorkspaceThreads.length}
                      </span>
                    </button>
                    {m.isHistoryOpen && (
                      <div className="ai-workspace-history-popover">
                        <div className="ai-workspace-history-head">
                          <span className="ai-workspace-history-label">
                            {m.aiCopy.composer.historyTitle}
                          </span>
                        </div>
                        {m.recentWorkspaceThreads.length === 0 ? (
                          <div className="ai-workspace-history-empty">
                            {m.aiCopy.composer.historyEmpty}
                          </div>
                        ) : (
                          <div className="ai-workspace-history-list">
                            {m.recentWorkspaceThreads.map((thread) => {
                              ...
                              return (
                                <div
                                  key={thread.id}
                                  className={`ai-workspace-history-item ${thread.id === m.currentThread?.id ? "is-active" : ""}`}
                                >
                                  {renamingThreadId === thread.id ? (
                                    ...
                                    <button
                                      ...
                                      onClick={() => m.selectThread(thread.id)}
                                    >
                                      <MessageSquareText className="ai-workspace-history-item-icon w-3.5 h-3.5" />
                                      <span className="ai-workspace-history-item-title">
                                        {memoryTitle || thread.label}
                                      </span>
                                  ...
                                  {renamingThreadId !== thread.id && (
                                    <div className="ai-workspace-history-item-actions">
                                      <button
                                        ...
                                        title={m.aiCopy.composer.historyRenameTitle}
                                        onClick={(event) => {
                                      ...
                                      <button
                                        type="button"
                                        className="ai-workspace-history-item-delete"
                                        title={m.aiCopy.composer.historyDeleteTitle}
                                        onClick={(event) => m.requestDeleteThread(thread.id, event)}
                                      >
                  ...
                  <button
                    ...
                    onClick={m.reloadChat}
                    disabled={m.isGenerating || m.isRunning}
                    title={m.aiCopy.composer.reloadChatTitle}
                  >
              ...
              <AIConversationView
                bubbles={m.conversationBubbles}
                copy={m.aiCopy}
                threadRef={m.chatThreadRef}
              ...
              <AIComposerDock
                copy={m.aiCopy}
                prompt={m.promptDraft}
                textareaRef={m.composerTextareaRef}
                footerNote={m.composerFooterNote}
                ...
                interactionMode={m.activeInteractionMode}
                ...
                sessionDataReadLabel={m.sessionDataReadButtonLabel}
                sessionDataReadTitle={m.sessionDataReadButtonTitle}
                showThinking={m.showThinking}
                onPromptChange={m.setPromptDraft}
                onKeyDown={m.composerKeyDown}
                slashMenu={m.slashMenu}
                onSelectSlashCommand={m.onSelectSlashCommand}
                onDismissSelection={m.dismissSelection}
                onSelectInteractionMode={m.selectInteractionMode}
                onSelectAgentAutonomy={handleSelectAgentAutonomy}
                onActivateProvider={m.activateProvider}
                onToggleModelVisibility={m.toggleModelVisibility}
                onSetSessionDataReadEnabled={m.setSessionDataReadEnabled}
                onSetShowThinking={m.setShowThinking}
                onOpenSettings={m.openSettings}
                safeModeEnabled={m.safeModeEnabled}
                sandboxPolicy={m.sandboxPolicy}
                onToggleSafeMode={m.onToggleSafeMode}
                onCloseHistory={() => m.setHistoryOpen(false)}
                onGenerate={m.generate}
                onCancelGeneration={m.cancelGeneration}
              />
            </div>
          </div>
        </aside>
      ...
      <ConfirmDialog
        isOpen={m.visualizationConsentPending !== null}
        title={m.visualizationConsentPending?.title || "Allow AI data read?"}
        message={m.visualizationConsentPending?.message || ""}
        confirmText={m.visualizationConsentPending?.confirmText || "Allow"}
        cancelText={m.visualizationConsentPending?.cancelText || "Deny"}
        onConfirm={() => m.confirmVisualizationConsent(true)}
        onCancel={() => m.confirmVisualizationConsent(false)}
      />
      <ConfirmDialog
        isOpen={m.destructiveConsentPending !== null}
        title={m.destructiveConsentPending?.title || "Confirm destructive action"}
        message={m.destructiveConsentPending?.message || ""}
        confirmText={m.destructiveConsentPending?.confirmText || "Confirm"}
        cancelText={m.destructiveConsentPending?.cancelText || "Cancel"}
        onConfirm={() => m.confirmDestructiveConsent(true)}
        onCancel={() => m.confirmDestructiveConsent(false)}
      />
      <ConfirmDialog
        isOpen={m.failoverConsentPending !== null}
        title={m.failoverConsentPending?.title || "Provider failed"}
        message={m.failoverConsentPending?.message || ""}
        confirmText={m.failoverConsentPending?.confirmText || "Allow auto-switch"}
        cancelText={m.failoverConsentPending?.cancelText || "Not now"}
        onConfirm={() => m.resolveFailoverConsent(true)}
        onCancel={() => m.resolveFailoverConsent(false)}
      />
      <ConfirmDialog
        isOpen={m.deleteThreadPending !== null}
        title={m.aiCopy.composer.historyDeleteTitle ?? "Delete conversation"}
        message={m.aiCopy.composer.historyDeleteConfirm ?? "Delete this conversation thread?"}
        confirmText="Delete"
        cancelText="Cancel"
        onConfirm={m.confirmDeleteThread}
        onCancel={m.cancelDeleteThread}
      />
      ...
      <AIAttachmentManager
        ...
        copy={m.aiCopy}
        onClose={m.closeAttachmentManager}
      />
