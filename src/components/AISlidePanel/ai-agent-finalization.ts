...
interface FinalizeAgentResultOptions {
  ...
  /** UI language for the validation-warning copy; defaults to English. */
...
function buildSteps(steps: AgentTraceStep[]): AIWorkspaceAgentStep[] {
  return steps.map((step) => ({
    ...
    status:
      step.observation.startsWith("Tool error") || step.observation.startsWith("Tool blocked")
        ? "error"
        : "done",
  }));
}
...
function buildWidgets(args: Record<string, unknown>): AIMetricsWidgetSpec[] {
  ...
  return rawWidgets
    .map((widget) => {
      const record =
        widget && typeof widget === "object" ? (widget as Record<string, unknown>) : {};
      ...
      const query =
        typeof record.query === "string"
          ...
          : typeof record.sql === "string"
            ...
            : "";
      const typeValue =
        typeof record.type === "string" ? record.type.trim().toLowerCase() : "table";
      const type = (
        [
          "table",
          "scoreboard",
          ...
          ? typeValue
          : "table"
      ) as AIMetricsWidgetSpec["type"];
      const dimension =
        typeof record.dimension === "string" ? record.dimension.trim() || undefined : undefined;
...
export async function finalizeAgentResult(
  options: FinalizeAgentResultOptions,
): Promise<AgentFinalization> {
  const {
  ...
  if (
    ...
    sqlResponseConflictsWithSchema(sql, availableSchemaTables)
  ) {
    agentTraceSteps.push({
      step: agentTraceSteps.length + 1,
      action: "finish",
      message: finalAction.message || "Final answer rejected.",
      observation:
        "Tool error: The proposed final SQL referenced tables outside the current workspace schema.",
    });
    ...
    finalAction =
      repaired.action === "finish"
        ? repaired
        : await recoverFinishAction(
  ...
  const finishResponseText =
    typeof finalAction.args?.response === "string" ? finalAction.args.response.trim() : "";
  if (finalAction.action !== "finish" || (!sql && !finishResponseText)) {
    ...
    const recoveredArgs: Record<string, unknown> = (recovered.args ?? {}) as Record<
      string,
      unknown
    >;
    const recoveredResponse =
      typeof recoveredArgs.response === "string" ? recoveredArgs.response.trim() : "";
    ...
    if (recovered.action === "finish" && (recoveredResponse || recoveredSql)) {
      ...
      finalAction = await recoverFinishAction(
        "The agent failed to produce a usable final answer after recovery.",
      );
    }
    sql = typeof finalAction.args?.sql === "string" ? finalAction.args.sql.trim() : "";
    if (sql) sql = extractSqlFromResponse(sql) || sql;
  ...
  if (sql && validateSql) {
    ...
    if (sqlValidationError) {
      ...
      if (repaired.action === "finish") {
        ...
        let repairedSql =
          typeof finalAction.args?.sql === "string" ? finalAction.args.sql.trim() : "";
  ...
  if (!sql && typeof args.response === "string") {
    ...
    if (
      fromResponse &&
      hasSqlStartKeyword(fromResponse) &&
      !(
        availableSchemaTables.length > 0 &&
        sqlResponseConflictsWithSchema(fromResponse, availableSchemaTables)
      )
    )
      sql = fromResponse;
  }
  ...
  const responseBody = sqlValidationError
    ? `${baseResponseBody}\n\n> ⚠️ **${formatPanelCopy(getAIPanelCopy(options.language ?? "en").sqlValidationWarning, { error: sqlValidationError })}**`
    : baseResponseBody;
  ...
  return {
    rawResponse,
    sql: shouldExposeSql ? sql : null,
    agentSteps: buildSteps(agentTraceSteps),
    agentWidgets: widgets.length ? widgets : undefined,
  };
}
