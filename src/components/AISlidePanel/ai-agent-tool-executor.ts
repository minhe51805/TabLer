...
export function createAgentToolExecutor(deps: AgentToolExecutorDeps) {
  ...
  try {
    ...
    if (action.action === "propose_seed_data") {
      ...
      if (toolAvailability && !toolAvailability.documentPropose && !toolAvailability.sqlWritePreview) {
      ...
      if (!collection) {
        return "Tool error: propose_seed_data requires args.collection — the exact table or collection name to fill.";
      }
      ...
      if (/[\s.$"']/.test(collection) || collection.startsWith("system.")) {
        return "Tool error: args.collection must be a plain table or collection name (no schema/db prefix, no whitespace, dots, quotes, or $ signs).";
      }
      ...
      if (isDocument) {
        ...
        seedScript = [
          "-- Seed data proposal generated by the AI agent — review before running.",
          generateInsertSql(collection, columns, rows, dbType),
        ].join("\n");
