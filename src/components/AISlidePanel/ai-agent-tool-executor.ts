...
export function createAgentToolExecutor(deps: AgentToolExecutorDeps) {
  ...
  try {
    ...
    if (action.action === "propose_seed_data") {
      // Document-engine seed proposals. The agent NEVER writes data itself:
      // this tool only lands a reviewable insertMany script in a NEW query tab
      // with autoRun disabled, so the human applies the write (Safe Mode will
      // still confirm before it runs).
      if (toolAvailability && !toolAvailability.documentPropose) {
        return `Tool blocked: propose_seed_data is only available on document engines (MongoDB). On SQL engines use preview_write to smoke-test INSERT statements, then propose them via edit_query_sql.`;
      }
      const collection = typeof action.args?.collection === "string" ? action.args.collection.trim() : "";
      if (!collection) {
        return "Tool error: propose_seed_data requires args.collection — the exact collection name to fill.";
      }
      // db.<name> shell syntax cannot carry whitespace, dots, or quotes.
      if (/[\s.$"']/.test(collection) || collection.startsWith("system.")) {
        return "Tool error: args.collection must be a plain collection name (no db. prefix, no whitespace, dots, quotes, or $ signs).";
      }
      const rawDocuments = action.args?.documents;
      if (!Array.isArray(rawDocuments) || rawDocuments.length === 0) {
        return "Tool error: propose_seed_data requires a non-empty args.documents array of objects.";
      }
      if (rawDocuments.length > AI_AGENT_SEED_DOCUMENT_LIMIT) {
        return `Tool error: args.documents exceeds the ${AI_AGENT_SEED_DOCUMENT_LIMIT}-document cap. Send the most representative documents.`;
      }
      const documents: Array<Record<string, unknown>> = [];
      for (const entry of rawDocuments) {
        if (!entry || typeof entry !== "object" || Array.isArray(entry)) {
          return "Tool error: every args.documents entry must be an object ({field: value}). Strings and arrays are not valid documents.";
        }
        const record = entry as Record<string, unknown>;
        if (Object.keys(record).length === 0) {
          return "Tool error: every args.documents entry must contain at least one field.";
        }
      ...
      const rationale = typeof action.args?.rationale === "string" ? action.args.rationale.trim() : "";
      const title = (rationale || `Seed proposal: ${collection}`).slice(0, 60);
      ...
      return stringifyAgentObservation({
        ...
        tabTitle: title,
        note: "Created a NEW AI Query tab pre-filled with the db." + collection + ".insertMany seed script. It is NOT auto-run: the user must review the tab and press Run (Safe Mode confirms). Never claim the data was inserted — you cannot run writes yourself. Call finish with a short summary of the proposal instead.",
      });
    }
