...
import { isSingleSqlStatement } from "../ai-panel-selection";
...
interface UseAIAssistantGenerationOptions {
  ...
  buildLoadingBubble: (
    prompt: string,
    options?: BuildLoadingBubbleOptions,
  ) => AIWorkspaceBubbleData;
  ...
  openMetricsBoardInWorkspace: (
    options?: OpenMetricsBoardOptions,
...
export function useAIAssistantGeneration({
  ...
  const createAssistantBubble = useCallback(
    async (
      prompt: string,
      options?: {
      ...
      const loadingBubble = buildLoadingBubble(normalizedPrompt, {
        mode: options?.mode,
        promptSummary:
          options?.displayPrompt?.trim() || summarizePromptForDisplay(normalizedPrompt),
                ...
                ...(thread.isAutoLabel
                  ? {
                      label: buildThreadLabel(
                        loadingBubble.promptSummary || normalizedPrompt,
                        index + 1,
                      ),
                      isAutoLabel: false,
                    }
      ...
      const directDashboardWidgetEdit = hasAttachedDashboardSelection
        ? resolveDashboardWidgetEditInstruction(
            requestPrompt,
            attachedSelection,
            dashboardEditConversationContext,
          )
        : null;
      ...
      try {
              ...
              : result.intent === "fix-error"
                ...
                : result.sql
                  ? result.risk?.level === "safe"
                    ? aiCopy.bubbleStates.readySqlSafeSubtitle
        ...
        if (agentCanAutoRun && result.sql) {
          try {
            const runResult = await runSql(result.sql, {
              agentAutonomy: activeAgentAutonomy,
              language,
            });
            setBubbles((current) =>
              current.map((bubble) =>
                bubble.id === loadingBubble.id
                  ? {
                      ...
                      detail: buildAIExecutionDetail(
                        runResult.summary,
                        runResult.queryResult.query,
                        result.rawResponse,
                      ),
              ...
              current.map((bubble) =>
                bubble.id === loadingBubble.id
                  ? {
                      ...
                      promptSummary: loadingBubble.promptSummary,
                      preview: message,
                      detail: buildAIAutoRunFailureDetail(
                        message,
                        result.sql || "",
                        result.rawResponse,
                      ),
        ...
        const wasCancelled =
          cancelledGenerationBubbleIdsRef.current.has(loadingBubble.id) ||
