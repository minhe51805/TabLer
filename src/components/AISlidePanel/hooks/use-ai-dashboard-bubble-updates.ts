...
export function formatDashboardWidgetType(type: MetricsWidgetType, useVietnamese: boolean) {
  const labels: Record<MetricsWidgetType, [vietnamese: string, english: string]> = {
    ...
    "stacked-bar": ["stacked bar chart", "stacked bar chart"],
    funnel: ["funnel chart", "funnel chart"],
    delta: ["delta metric", "delta metric"],
    markdown: ["markdown card", "markdown card"],
  };
...
export function useAIDashboardBubbleUpdates({
  ...
  const updateBubbleForDashboardNoChange = useCallback(
    (bubbleId: string, promptText: string, addedCount = 0) => {
      ...
      const detail = useVietnamese
        ? [
            ...
            addedCount > 0
              ? `So widget vua them: ${addedCount}.`
              : "Chua co thay doi chart nao duoc ap dung.",
            "",
        ...
        : [
            ...
            addedCount > 0
              ? `Widgets added just now: ${addedCount}.`
              : "No chart changes were applied.",
            "",
          ...
          bubble.id === bubbleId
            ? {
                ...
                subtitle: useVietnamese
                  ? "Khong co chart moi duoc ap dung"
                  : "No new chart changes were applied",
                preview,
                detail,
                sql: undefined,
                risk: undefined,
  ...
  const updateBubbleForDashboardActionFailed = useCallback(
    (bubbleId: string, promptText: string, reason?: string) => {
      const useVietnamese = prefersVietnameseSystemReply(promptText, language);
        ...
        current.map((bubble) =>
          bubble.id === bubbleId
            ? {
                ...
                status: "error",
                title: useVietnamese
                  ? "Thao tac dashboard khong thanh cong"
                  : "Dashboard action failed",
                subtitle: useVietnamese
                  ? "Khong ap dung thay doi nao"
                  : "No dashboard change was applied",
                preview,
                detail,
                sql: undefined,
                risk: undefined,
  ...
  const updateBubbleForDashboardEditNeedsClarification = useCallback(
    (bubbleId: string, promptText: string) => {
      const useVietnamese = prefersVietnameseSystemReply(promptText, language);
      const preview = useVietnamese
        ...
        current.map((bubble) =>
          bubble.id === bubbleId
            ? {
                ...
                subtitle: useVietnamese
                  ? "Chua sua dashboard vi chua map duoc widget"
                  : "Dashboard edit was not applied yet",
                preview,
                detail,
                sql: undefined,
                risk: undefined,
  ...
  const updateBubbleForAttachedDashboardSummary = useCallback(
    (bubbleId: string, promptText: string, selection: VisualizationSelectionContext) => {
      const useVietnamese = prefersVietnameseSystemReply(promptText, language);
      ...
      const detail = useVietnamese
        ...
        : [
            ...
            remainingCount > 0
              ? `${remainingCount} more widget(s) are also present on this board.`
              : "",
            "",
        ...
        current.map((bubble) =>
          bubble.id === bubbleId
            ? {
                ...
                subtitle: useVietnamese
                  ? "San sang sua truc tiep tren board hien tai"
                  : "Ready to edit the current board directly",
                preview,
                detail,
                sql: undefined,
                risk: undefined,
  ...
  const updateBubbleForDashboardApplied = useCallback(
    (bubbleId: string, promptText: string, addedCount = 0, addedTitles: string[] = []) => {
  ...
  const updateBubbleForDashboardEdited = useCallback(
    (bubbleId: string, promptText: string, widgetTitle: string, nextType: MetricsWidgetType) => {
      const useVietnamese = prefersVietnameseSystemReply(promptText, language);
  ...
  const updateBubbleForDashboardRebuilt = useCallback(
    (bubbleId: string, promptText: string, widgetCount = 0, widgetTitles: string[] = []) => {
