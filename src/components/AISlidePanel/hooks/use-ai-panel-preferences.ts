...
interface UseAIPanelPreferencesOptions {
  ...
  setWorkspaceInteractionModes: Dispatch<
    SetStateAction<Record<string, AIWorkspaceInteractionMode>>
  >;
}
...
export function useAIPanelPreferences(options: UseAIPanelPreferencesOptions) {
  ...
  const selectInteractionMode = useCallback(
    (mode: AIWorkspaceInteractionMode) => {
      setWorkspaceInteractionModes((current) => ({ ...current, [currentWorkspaceKey]: mode }));
    },
    [currentWorkspaceKey, setWorkspaceInteractionModes],
  );
  const selectAgentAutonomy = useCallback(
    (autonomy: AIWorkspaceAgentAutonomy) => {
      setWorkspaceAgentAutonomy((current) => {
        // Dropping the "full" grant restores the Safe Mode level its consent
        // dialog suspended — protection comes back with the grant, never later.
        if (current[currentWorkspaceKey] === "full" && autonomy !== "full") {
          useSafeModeStore.getState().resumeSafeModeFromFullAccess();
        }
        return { ...current, [currentWorkspaceKey]: autonomy };
      });
    },
        ...
        .then(async () => {
          ...
          try {
            const saved = (await saveAIConfigs(nextConfigs, {}, [])) as
              { aiConfigs?: AIProviderConfig[] } | undefined;
      ...
      const nextConfigs = normalizeAIProviderConfigs(
        aiConfigs.map((config) =>
          config.id === providerId
            ? {
                ...config,
                disabled_models: (config.disabled_models ?? []).filter((entry) => entry !== model),
              }
            : config,
        ),
      );
