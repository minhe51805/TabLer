/**
 * SchemaDiffPanel — the schema-diff tool body (connection/database pickers,
 * compare, grouped results, migration-script generation). Hosted by the
 * standalone SchemaDiffView modal and by the Review Center's Schema diff tab.
 */
...
/** Last picker selections — the panel unmounts when its host closes, so the
 *  previous choice is remembered module-wide and restored on the next open. */
let lastSelection: { connectionA: string; connectionB: string } | null = null;
...
export function SchemaDiffPanel() {
  ...
  const [connectionA, setConnectionA] = useState<string>(
    () => lastSelection?.connectionA || activeConnectionId || "",
  );
  const [connectionB, setConnectionB] = useState<string>(() => lastSelection?.connectionB ?? "");
  ...
  // Remember the picker selection for the next mount (modal or review tab).
  const selectionRef = useRef({ connectionA, connectionB });
  selectionRef.current = { connectionA, connectionB };
  useEffect(
    () => () => {
  ...
  return (
    <div className="schema-diff-body">
                ...
                {group.tables.map((table) => (
                  ...
                  >
                    ...
                    {table.columns.map((column) => (
                      ...
                      >
                        {column.change === "added" ? "+" : column.change === "removed" ? "-" : "~"}{" "}
                        {column.name}
