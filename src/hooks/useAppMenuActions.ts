...
export function useAppMenuActions(handlers: AppMenuActionHandlers) {
  ...
  return useMemo(
    () => ({
      ...
      onOpenServerInfo: () => handleOpenAdminQuery("server-info"),
      onOpenLocks: () => handleOpenAdminQuery("locks"),
      onOpenTableStats: () => handleOpenAdminQuery("table-stats"),
      onOpenIndexUsage: () => handleOpenAdminQuery("index-usage"),
      onOpenSlowQueries: () => handleOpenAdminQuery("slow-queries"),
